...
const STYLES = `  :root{
  ...
  *{box-sizing:border-box;}
  ...
  @media (max-width:960px){
    .layout, .layout.layout--empty{grid-template-columns:1fr; gap:32px;}
    .layout.layout--empty .right-col{display:none;}
  }
  ...
  .upload-card-inner{padding:16px;}
  .upload-head{margin:0 0 0;}
  .upload-title{margin:0;font-weight:700;font-size:15px;display:none;}
  .upload-sub{margin:4px 0 0;font-size:13px;color:var(--ink-soft);font-weight:400;display:none;}
  ...
  .dz-line1{margin:0;font-size:13px;color:var(--ink-soft);order:2;}
  .dz-line2{margin:0;font-size:20px;font-weight:800;text-transform:uppercase;color:var(--ink);order:1;display:flex;align-items:center;justify-content:space-between;gap:10px;}
  .dz-browse{color:var(--ink);text-decoration:none;font-weight:800;cursor:pointer;}
  .dz-add{
  ...
  .dz-empty{
    ...
    gap:10px; color:var(--ink-faint); text-align:center;
  }
  ...
  /* ---------- design list ---------- */
  .design-list{background:var(--file-row); border-top:1px solid var(--line);}
  .design-row{
    display:flex;
    align-items:center;
    gap:12px;
  ...
  .design-row.active{background:var(--surface-muted);}
  .design-thumb{
    flex:none;
  ...
  .design-name{margin:0;font-size:13.5px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--ink);}
  ...
  /* ---------- Right column ---------- */
  .right-col{display:flex;flex-direction:column;gap:24px;padding-top:0;min-width:0;}
  .tabs{
    display:inline-flex;
    align-items:center;
    gap:6px;
  ...
  /* ---------- Horizontal board of panel cards ---------- */
  .board{
    display:flex;
    gap:20px;
    overflow-x:auto;
  ...
  .board::-webkit-scrollbar-thumb{background:var(--line-strong); border-radius:8px;}
  .board-card{
    flex:0 0 340px;
  ...
  .num-index{font-family:"Inter", system-ui, sans-serif; font-size:14px; color:var(--ink-soft); margin:0 0 6px; font-weight:700;}
  .num-tag{
          ...
          <div className="left-col">
            ...
            <div className="upload-card">
              <div className="upload-card-inner">
                ...
                <div
                  className={"dropzone" + (s.drag ? " drag" : "") + (active ? " has-preview" : "")}
                  tabIndex={0}
                  role="button"
                ...
                >
                  ...
                  {active ? (
                    <>
                      <img className="dz-preview" src={active.url} alt={active.name} />
                      <div
                        className="dz-add"
                        role="button"
                        aria-label="Upload another design"
                        onClick={(e) => {
                          e.stopPropagation();
                          fileRef.current && fileRef.current.click();
                        }}
          ...
          <div className="right-col">
            ...
            {s.activeTab === "compare" ? (
              ...
              <div className="board" ref={boardRef}>
                {["pm", "eng", "skeptic", "summary"].map((key) => {
                  const on = s.activeTab === key;
                  return (
                    <div
                      key={key}
                      ref={(el) => {
                      ...
                      className={"board-card" + (on ? " active" : "")}
                    >
                      {renderContent(key)}
                    </div>
                  );
